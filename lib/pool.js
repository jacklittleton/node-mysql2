...
class Pool extends EventEmitter {
  ...
  releaseConnection(connection) {
    ...
    if (!connection._pool) {
      ...
      if (connection._creationTime && this.config.ttl > 0 && connection._creationTime <= Date.now() - this.config.ttl) {
        console.log(`Connection id ${connection.connectionId} should be collected`);
        spliceConnection(this._allConnections, connection);
        connection._pool._removeFromPool();
        connection.destroy();
